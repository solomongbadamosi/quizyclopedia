...
class Mysubscription {
    ...
    private function _subscription_prep($post_data){
        ...
        clear_existing_reference($prv_amount->amount, $post_data['amount']);
        if($prv_amount && $prv_amount->amount!=$post_data['amount']){
            if($this->CI->form_validation->run('subscription')==TRUE){
                 //get post data
                 $post_data = subscriptionpost($this->CI->session->time);
                 //clear existing references from database
                 clear_existing_reference($prv_amount->amount, $post_data['amount']);
             }else{
                 print_r('show a view for asking user to choose an option between new and old payment');
             }
        }
    }
    ...
    private function _subscription_check($result, $existing){
        if($result){
            if($result['data'] && $result['data']['status'] == 'success'){
                ...
                $this->CI->session->set_flashdata('success_msg', $data['msg']);
                redirect('user/login');
    ...
    private function _subscription_amount($result){
        if($result['data']['amount']===1000000){
                $data = array('subscribed'=>'1', 'expiry_time'=> time()+31622400, 'amount'=>10000, 
                        'msg' => 'Congratulations, You are an Active User for another 365 Days');
            }elseif($result['data']['amount']===500000){
                $data = array('subscribed'=>'1', 'expiry_time'=> time()+15811200, 'amount'=>5000, 
                        'msg' => 'Congratulations, You are an Active User for another 6 months');
            }elseif($result['data']['amount']===400000){
                $data = array('subscribed'=>'1', 'expiry_time'=> time()+10540800, 'amount'=>4000, 
                        'msg' => 'Congratulations, You are an Active User for another 4 months');
            }else{
                $data = array('subscribed'=>'1', 'expiry_time'=> time()+2635200, 'amount'=>2000, 
                        'msg' => 'Congratulations, You are an Active User for another 1 month');
            }
        return $data;
    }
    ...
    private function _subscription_share($data, $referral){
        ...
        if($referral){
            ...
            $share = distribute_commission($data['amount'], $previous['referral']['amount'], $previous['school']['amount']);
            //update school's share
            if($share['school']!=''){
                $this->CI->schools->update(array('amount'=>$share['school']), $referral->school_id);
            }elseif($share['referral']!='') {
